...
class payment_main():
    def __init__(self):
        ...
        self.iousidr.resize(220, 35)
        self.iousidr.setPlainText("欠单号:")
        self.iousidr.setReadOnly(True)
        self.iousid = QPlainTextEdit(self.window)
        self.iousid.move(20, 90)
        self.iousid.resize(220, 35)
        ...
        more_space = ' '*62
        self.searchtitle.setPlainText(f"{more_space}检索非已付清的欠条信息{more_space}")
    ...
    def uploadpayment(self):
        ...
        ious_id = self.iousid.toPlainText().strip()

        remark = self.remark.toPlainText().strip()


        pay = afunc.create_notlistpayment(user_name, date, client, amount, ious_id, remark)
        ...
        if valid == True: #成功
            ...
            afunc.put_history(f'尝试对{ious_id}进行了一次支付，失败\n')
