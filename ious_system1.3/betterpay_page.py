from PySide2.QtWidgets import QApplication, QMainWindow, QPushButton, QPlainTextEdit,  QScrollArea, QWidget, QVBoxLayout,QTableWidget, QCheckBox, QTableWidgetItem
...
class better_pay():
    def __init__(self):
        ...
        self.cur = []; #self.searchious() #目前检索到的所有欠单
        self.sel = [] #目前选择的所有欠单
        self.checkboxpair = []
        ...
        space = ' '*67
        self.handaddtitle.setPlainText(f"{space}多笔载入付款信息{space}")
        ...
        self.fscroll_area.resize(1360, 340)
        ...
        self.fshowpay = QTableWidget()
        self.fshowpay.setAlternatingRowColors(True)
        self.inner_layout.addWidget(self.fshowpay)
        self.fshowpay.setColumnCount(4)
        self.fshowpay.setRowCount(0)

        self.fshowpay.setColumnWidth(0,60)       
        self.fshowpay.setColumnWidth(1,130)
        self.fshowpay.setColumnWidth(2,260)
        self.fshowpay.setColumnWidth(3,1000)
        self.fshowpay.setHorizontalHeaderLabels(['','日期', '单号', "内容"])
    

        self.allselect = QCheckBox('全选',self.window)
        self.allselect.move(20,830)
        self.allselect.stateChanged.connect(lambda: self.allchanged())

        self.searchrestr = QPlainTextEdit(self.window)
        self.searchrestr.move(100, 830)
        self.searchrestr.resize(1200, 35)
        self.searchrestr.setPlainText("总单数: \t\t\t\t总金额: ")
        self.searchrestr.setReadOnly(True)
    ...
    def uploadpayment(self):
        ...
        if amount<0:
            old_text = self.pshowpay.toPlainText()
            self.pshowpay.setPlainText(old_text+'请输入非负数金额\n')
            return 
        ...
        #此时total(包含正负总和) 则必然 >= 实际支付金额，因此没有问题
        afunc.put_history(f'对多笔欠单进行了金额为{amount}的一次支付，成功，如下\n')
        if total == amount: #两者必然全清，做特殊处理
            for business in self.sel:
                pay = afunc.create_notlistpayment(user_name, date, client, business.rest, business.ious.id, remark)
                #无论如何都设置为0
                amount -= business.rest #用于末尾检查
                ...
                if valid == True: #成功
                    ...
                    break
            if amount == 0: #实际应该百分百触发
                past = self.pshowpay.toPlainText()
                new = '本次对所选所有欠条均清零\n'
                self.pshowpay.setPlainText(past+new)

        elif total > amount: #实际必然进入这个循环
            #重新统计总金额
            total = 0
            for business in self.sel:
                if business.rest > 0: 
            ...
            #此时total为总共的需要支付的正金额，而amount为本来用户提供的金额+付欠条的金额
            for business in self.sel:
                if business.rest <=0:               #无条件清空
                    pay = afunc.create_notlistpayment(user_name, date, client, business.rest, business.ious.id, remark)
                    #无论如何都设置为0
                    valid = afunc.payment_to_business(pay)
                elif amount<=0 and business.rest > 0: #需要支付且没钱
                    continue
                else: #amount >0, business.rest >0
    ...
    def displayious(self):
        if len(self.sel) + len(self.cur) >= 500:
            self.searchtitle.setPlainText(f"超过500条! 不予显示")
            return 
        self.fshowpay.setRowCount(0)
        ...
        for i in range(len(self.sel)):
            business = afunc.get_business_byid(self.sel[i].ious.id)
            self.sel[i] = business
            keys.append(business.ious.id)

            self.fshowpay.insertRow(self.fshowpay.rowCount())

            checkbox = QCheckBox()
            checkbox.setChecked(True)
            checkbox.stateChanged.connect(lambda: self.btnstate())


            self.checkboxpair.append((checkbox,business))

            # 为每一行添加复选框
            self.fshowpay.setCellWidget(row,0,checkbox)

            self.fshowpay.setItem(row,1,QTableWidgetItem(business.ious.date))
            self.fshowpay.setItem(row,2,QTableWidgetItem(business.ious.id))
            self.fshowpay.setItem(row,3,QTableWidgetItem(afunc.convert_key_business_text(business)))
            self.fshowpay.resizeRowsToContents()
            row+=1

        
        #展示剩余
        for business in self.cur:
            if business.ious.id in keys:
                continue
    ...
    def btnstate(self):
        self.sel = []
        for (checkbox,business) in self.checkboxpair:
            if checkbox.isChecked():
                self.sel.append(business)
        
        #测试
        for busi in self.sel:
            print(busi.ious.id)
        self.update_val()
    ...
    def allchanged(self):
        ...
        if self.allselect.isChecked(): #全是
            for (checkbox,business) in self.checkboxpair:
                checkbox.setChecked(True)
    ...
    def update_val(self):
        ...
        for business in self.sel:
            ct+= 1
            amount += business.rest
        self.searchrestr.setPlainText(f"总单数:{ct}单 \t\t\t\t总金额:{amount}元 ")
